...
class Menus_API {
  ...
  public function register_routes() {
    ...
    register_rest_route( self::get_plugin_namespace(), '/category', array(
      array(
        'methods'  => WP_REST_Server::READABLE,
        'callback' => array( $this, 'get_menu_posts' ),
      )
    ) );
  }

  /**
   * Get menus posts from category 
   *
   * es: treasure-island ( category slug )
   *           - tag chapter ( chapter-1, chapter-2, chapter-3..)
   *           - tag lang [eng,ita]
   *
   * @since  1.2.0
   * @return array of posts
   */
  function get_menu_posts(\WP_REST_Request $request) {

    $category_slug = $request['category_slug']; // es slug 'treasure-island'

    $categories = get_categories(array('slug' => $category_slug));
